...
Using STL with Box-Cox, STL+Naive slightly outperformed STL+Drift, but both had higher errors than the Naive benchmark from Question 3. This is likely due to the test set's unusually sharp decline, which decomposition models struggled to track. For this specific period, Naive remains preferred.
...
Time series cross-validation selected STL_Naive as the best method. The cross-validation results differed from the test set results that the test set selected Naive, while cross-validation favoured STL_Naive. This suggests that STL_Naive performed better on average across the whole series, but Naive worked better in the final two years. **We therefore prefer STL_Naive**, as cross-validation provides a broader assessment of forecast performance. Its main advantage is using many rolling forecast origins rather than one split. However, it is more computationally expensive and may reflect the most recent period less closely than a single test set.
