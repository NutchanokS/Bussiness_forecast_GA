...
The results in @fig-q4-forecast and @tbl-q4-accuracy show that using STL with Box-Cox, STL+Naive slightly outperformed STL+Drift, but both had higher errors than the Naive benchmark from Question 3 over this test period. This is likely due to the test set's unusually sharp decline, which decomposition models struggled to track. For this specific period, Naive remains preferred.
...
```{r}
#| label: tbl-q5-tscv
#| tbl-cap: "Time series cross-validation accuracy comparison."
...
The cross-validation results in @tbl-q5-tscv select STL_Naive as the best method. The cross-validation results differed from the test set results that the test set selected Naive, while cross-validation favoured STL_Naive. This suggests that STL_Naive performed better on average across the whole series, but Naive worked better in the final two years. **We therefore prefer STL_Naive**, as cross-validation provides a broader assessment of forecast performance. Its main advantage is using many rolling forecast origins rather than one split. However, it is more computationally expensive and may reflect the most recent period less closely than a single test set.
...
#| fig-cap: "24-month ahead point forecasts and 80%/95% prediction intervals from STL_Naive, fitted on the training set."
