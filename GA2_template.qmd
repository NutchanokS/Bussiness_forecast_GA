...
The cross-validation results were different from the test set results. The test set selected Naive as the best method, but cross-validation favoured STL_Naive. This suggests that STL_Naive performed better on average across the whole series, while Naive worked better over the final two-year period.

**Advantages/Disadvantages**

An advantage of time series cross-validation is that it uses many rolling forecast origins, so it provides a broader and less split-dependent assessment than a single test set. However, it is more computationally expensive, and the results may differ from the final test set because they average performance over many periods rather than focusing only on the most recent observations.
