...
The cross-validation results were broadly similar to the test set results, suggesting that the better-performing methods were reasonably consistent. However, the exact error values differed because cross-validation evaluates performance over many forecast origins, whereas the test set assesses performance only over one limited final period.

**Advantages/Disadvantages**

The advantage of time series cross-validation is that it uses many rolling forecast origins, so it provides a broader and less split-dependent assessment than a single test set. However, it is more computationally expensive, and because it averages performance over many periods, it may reflect the most recent observations less closely than the final test set.
